  ...
  return (
    <div className={`fixed bottom-6 right-6 z-50 ${className}`}>
      <Button
        ...
        disabled={disabled}
        size="lg"
        className="h-12 px-6 rounded-full bg-finance-green hover:bg-finance-green-dark shadow-lg font-medium transition-all hover:shadow-xl flex items-center gap-2"
      ...
      >
        ...
        <span>{label}</span>
        {keys.length > 0 && (
          <span className="flex items-center gap-1 ml-1 opacity-90" aria-hidden="true">
            {keys.map((key, index) => (
