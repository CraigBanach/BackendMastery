...
public class BudgetService : IBudgetService
{
    ...
    public async Task<IEnumerable<BudgetDto>> GetBudgetsForMonthAsync(string userId, int year, int month)
    {
        var budgets = await GetOrCreateBudgetsForMonthAsync(userId, year, month);
    ...
    public async Task<IEnumerable<BudgetDto>> SetBudgetsForMonthAsync(string userId, int year, int month, IEnumerable<SetBudgetDto> budgets)
    {
        ...
        var updatedBudgets = await _budgetRepository.SetBudgetsForMonthAsync(userId, year, month, setBudgetRequests);
    ...
    public async Task<BudgetDto?> GetBudgetAsync(string userId, int categoryId, int year, int month)
    {
        var budget = await _budgetRepository.GetBudgetAsync(userId, categoryId, year, month);
    ...
    private async Task<decimal> GetActualSpendingAsync(string userId, int categoryId, int year, int month)
    {
    ...
    private async Task<IEnumerable<Budget>> GetOrCreateBudgetsForMonthAsync(string userId, int year, int month)
    {
        var existingBudgets = await _budgetRepository.GetBudgetsForMonthAsync(userId, year, month);
        ...
        var previousMonthBudgets = await FindPreviousMonthBudgets(userId, year, month);
        ...
        return await _budgetRepository.SetBudgetsForMonthAsync(userId, year, month, copyForwardRequests);
    }

    private async Task<IEnumerable<Budget>> FindPreviousMonthBudgets(string userId, int year, int month)
    {
        ...
        for (int i = 0; i < 12; i++)
        {
            var budgets = await _budgetRepository.GetBudgetsForMonthAsync(userId, targetDate.Year, targetDate.Month);
            if (budgets.Any())
            {
                return budgets;
            }
